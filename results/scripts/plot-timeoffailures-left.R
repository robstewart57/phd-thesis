...
#   x <- sqldf(paste("select mean from cdata where TimeOfFail == 1 and Variant = '", variant, "'", sep=""))
#   return(x)
# }
...
# show runtime
bp <- ggplot(cdata, aes(x=TimeOfFail, y=mean, group=Variant)) + geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1) + geom_line(aes(linetype=Variant)) + geom_point(aes(shape=Variant)) + ylab("Runtime (Seconds)") + xlab("Time of Simultanous 5 Node Failure (Seconds)") + labs(title = paste("Runtime for",title,sep=" ")) + geom_hline(yintercept = as.numeric(failureFreeLazy), linetype="dashed") + annotate("text", min(21), as.numeric(failureFreeLazy)+3, label = failureFreeLazySkel,size=3) + geom_hline(yintercept = as.numeric(failureFreeEager), linetype="dashed") + annotate("text", min(21), as.numeric(failureFreeEager)+3, label = failureFreeEagerSkel,size=3)

print(bp + theme_bw()  + theme(legend.justification=c(1,1), legend.position=c(0.5,1), legend.key.width=unit(3,"line")))
